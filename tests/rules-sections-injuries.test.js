...
test('INJ revocation before final write denies request',()=>run(async db=>{let n=0;const f=global.fetch;global.fetch=(url,opts)=>{if(String(url).includes('sohail_account_security')&&++n===2)db.state().users.admin.inactive=true;return f(url,opts);};assert.equal((await call(liga,q(db))).status,401);assert.equal(writes(db).length,0);}));
