...
test('RT450 version and mandatory 50-window are explicit',()=>{assert.equal(E.VERSION,'sohail-rating-4.5.0');assert.equal(E.DEFAULTS.window,50);assert.throws(()=>calc([match(1)],{window:49}),e=>e.code==='INVALID_WINDOW');});
...
test('RT450 new explainer is present before the UI in the shipped page',()=>{const html=fs.readFileSync(path.join(__dirname,'../public/index.html'),'utf8');assert.ok(html.indexOf('rating-explainer.js?v=sohail-v450-rating')<html.indexOf('rating.js?v=sohail-v450-rating'));assert.ok(html.includes('sohail-v450-rating'));});
