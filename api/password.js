...
module.exports=require('./_http').wrap(async function(req,res){
 ...
 const liga=lib.resolveLigaId(body.ligaId);
 ...
 if(!other&&!account.must_change){
   ...
   const valid=old.length<=128&&(lib.verifyPassword(stored,old)||(name==='superadmin'&&lib.SUPER_HASH&&lib.hashV2(old)===lib.SUPER_HASH));
