...
function _guideLayout(){
 ...
 const mobile=w<760,margin=mobile?8:22,gap=mobile?8:14;
 const width=Math.min(mobile?Math.max(292,w-16):400,w-20);
 const clamp=(n,min,max)=>Math.max(min,Math.min(max,n));
 dialog.style.width=width+'px';
 dialog.style.maxHeight=Math.max(170,h*(mobile?0.46:0.72))+'px';
