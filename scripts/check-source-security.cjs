...
function check(root=path.resolve(__dirname,'..')){
 ...
 if(v.public!==false||v.outputDirectory!=='dist'||v.buildCommand!=='npm run release:check')failures.push({file:'vercel.json',type:'publication-boundary'});
...
module.exports={check,secretFindings};
