...
msg_placeholder:'Escribe un mensaje…',msg_attach:'Adjuntar imagen',msg_attach_remove:'Quitar imagen',msg_attach_too_big:'La imagen es demasiado grande (máximo 8 MB).',msg_attach_err:'No se pudo procesar la imagen.',
msg_send:'Enviar',
...
msg_placeholder:'Write a message…',msg_attach:'Attach image',msg_attach_remove:'Remove image',msg_attach_too_big:'The image is too large (max 8 MB).',msg_attach_err:'Could not process the image.',
msg_send:'Send',
...
msg_explorar_desc:'Admins only: pick a cycle and a group (or a Play Offs bracket) to read its chat, and you can also write there.',
...
// Ajustes manuales del admin sobre el puntaje final de un jugador puntual,
// en un ciclo y grupo puntuales — para penalidades o bonus discrecionales
// que no dependen de la posición en la tabla (eso ya lo cubre PUNTOS,
// arriba). Estructura: { [ciclo]: { [grupo]: { [nombreJugador]: numero } } }.
// El número puede ser negativo (penalidad) o positivo (bonus). Se suma al
// Total de la fila del jugador en groupCardHTML (shell-render.js), junto a
// base (ptsForPos) y ex (el +2 fijo al ganador del grupo).
let AJUSTES_PUNTOS={};
...
// Nombre OFICIAL de la liga tal como figura en liga_index (lo mismo que
// "Gestión de ligas"). Lo manda el servidor en cada login/state (campo
// ligaNombre) y NO se edita desde ningún formulario — solo cambia con el
// botón "Renombrar" de Gestión de Ligas, que sincroniza ambos lados. Es la
// fuente de verdad para el header (hdr-title) y el título del login post-
// selección, justamente para que ese nombre visible nunca se desincronice
// de lo que ves en Gestión de Ligas. Si el servidor no lo manda (por
// ejemplo, una versión vieja de la API), cae a LEAGUE_NAME.
let LIGA_NOMBRE_OFICIAL='';
...
function computeStats(cycN,gid){
  ...
  confirmed.forEach(m=>{
    ...
    // Retiro (RET/walkover): el GANADOR del partido siempre viene explícito
    // en m.winner (no se deduce de los sets — alguien puede ir arriba en
    // sets y retirarse igual, perdiendo el partido a pesar de eso). Los
    // sets que sí se hayan jugado (m.sets puede traer 0, 1 o 2 sets reales)
    // SÍ se suman a sg/sp/gw/gl como cualquier otro partido — importan para
    // el desempate por balance de sets/games y para el rating. Antes esto
    // ignoraba los sets por completo en un retiro, aunque hubiera sets
    // reales jugados antes de retirarse.
    if(m.wo){
  ...
  function h2hStats(group){
    ...
    confirmed.forEach(m=>{
      ...
      // arriba — el ganador viene de m.winner, pero los sets reales
      // jugados (si los hay) también se suman a sg/sp/gw/gl.
      if(m.wo){
...
function computeGeneral(){
  ...
  cycles.forEach(c=>{
    ...
    c.groups.forEach((g,gi)=>{
      ...
      st.forEach((s,pos)=>{
        // v incluye el ajuste manual del admin (AJUSTES_PUNTOS) — mismo
        // cálculo que hace groupCardHTML (shell-render.js) para el Total
        // de la tabla de cada grupo, así la Clasificación General no
        // queda desincronizada de lo que se ve en cada grupo puntual.
        const gid=gi+1;
        const ajuste=(AJUSTES_PUNTOS[c.n]&&AJUSTES_PUNTOS[c.n][gid]&&AJUSTES_PUNTOS[c.n][gid][s.name])||0;
        const v=ptsForPos(gid,pos)+(pos===0?2:0)+ajuste;
  ...
  // Segunda pasada: ajustes "sueltos" (AJUSTES_PUNTOS[ciclo][0][nombre]) —
  // gid 0 es la convención para "el admin le dio puntos en este ciclo sin
  // que el jugador haya participado ahí" (mismo valor que ya usa el resto
  // del sistema para "Sin grupo", ver findLoc()/estadoInicial). El loop de
  // arriba SOLO suma jugadores que aparecen en computeStats() de un grupo
  // real — un jugador sin ningún partido en un ciclo nunca entra ahí, así
  // que sin esta segunda pasada, un ajuste guardado para él simplemente no
  // se sumaría nunca a ningún Total. Acá se suma explícitamente, incluso
  // para un jugador que no tiene NINGÚN punto de partido en absoluto
  // (tot[nombre] puede no existir todavía — se crea en 0 antes de sumar).
  cycles.forEach(c=>{
    const ajustesSueltos = AJUSTES_PUNTOS[c.n] && AJUSTES_PUNTOS[c.n][0];
    if(!ajustesSueltos) return;
    Object.keys(ajustesSueltos).forEach(nombre=>{
      // Solo jugadores reales de la liga — evita que un nombre mal escrito
      // o un jugador borrado deje un ajuste huérfano sumando puntos fantasma.
      if(!USERS[nombre]) return;
      const v = ajustesSueltos[nombre] || 0;
      if(!v) return;
      tot[nombre]=(tot[nombre]||0)+v;
      if(!porCiclo[nombre])porCiclo[nombre]={};
      porCiclo[nombre][c.n]=(porCiclo[nombre][c.n]||0)+v;
    });
  });
...
function rebuildTramo(t){
  ...
  // mainOrder: mismo mecanismo que consOverrides._order (ver más abajo, para
  // consolación), pero para el cuadro PRINCIPAL — un array de SLOTS
  // (incluye null en las posiciones BYE, tamaño fijo = tamaño del cuadro)
  // que representa el orden visual final tal como el admin lo dejó a mano.
  // Sin esto, el admin podía reordenar QUIÉN juega contra quién (moveSeedUpUI/
  ...
  if(tieneMainOrderManual){
    // Igual que se hace para consolación: el sid (número de sembrado
    // mostrado junto al jugador) tiene que reflejar SIEMPRE el ranking
    // real de Clasificación General (tr.seeds, sin reordenar) — no la
    // posición dentro del array ya reordenado a mano, que es lo que
    // buildRounds calculó por default (ya que ahí no tiene forma de saber
    // cuál de los dos árboles es "el ranking real").
    tr.main[0].forEach(m=>{
      if(m.a) m.sid[0] = tr.seeds.indexOf(m.a)+1 || '';
      if(m.b) m.sid[1] = tr.seeds.indexOf(m.b)+1 || '';
    });
  }
  applyStored(t,tr.main);
  let losers=tr.main[0].map(loserOf).filter(Boolean);
  // consOverrides: personalización manual del admin sobre la consolación.
  // Tres cosas distintas conviven en el mismo objeto:
  ...
  //   - '_extra': array de jugadores agregados SIN reemplazar a nadie —
  //     por ejemplo alguien que ganó octavos y perdió en cuartos, que el
  //     admin quiere sumar a consolación además de (no en lugar de) los
  //     perdedores reales de primera ronda. Se suman después de los
  //     reemplazos; el cuadro crece a la potencia de 2 que corresponda
  //     (ver numRealesConsolacion más abajo — buildRounds ya sabe agregar
  //     los BYE que hagan falta). Es una clave reservada, nunca puede
  //     coincidir con un nombre de jugador real.
  //   - '_order': array explícito con el ORDEN final de esos jugadores
  //     (después de reemplazos y _extra) — permite mover a alguien de
  //     llave dentro de consolación sin cambiar quién participa, algo que
  //     los reemplazos y _extra por sí solos no cubren. Es una clave
  //     reservada, nunca puede coincidir con un nombre de jugador real.
  ...
  const reemplazos = Object.keys(ov).filter(k=>k!=='_order'&&k!=='_extra');
  if(reemplazos.length){
    losers = losers.map(n => (ov[n] !== undefined) ? ov[n] : n)
                    .filter(n => n !== null);   // null = "sacar sin reemplazo"
  }
  // '_extra': jugadores sumados a consolación sin sacar a nadie. Se filtra
  // cualquiera que ya esté en `losers` (evita duplicar si, por ejemplo, el
  // admin lo agregó a mano y además terminó perdiendo primera ronda en un
  // recálculo posterior del cuadro principal).
  if(Array.isArray(ov._extra) && ov._extra.length){
    ov._extra.forEach(n=>{ if(n && !losers.includes(n)) losers.push(n); });
  }
...
async function cambiarLigaDesdeMenu(ligaId){
  ...
  try{
    ...
    _loadOK = true;
    // Nombre oficial de la liga a la que se cambió (el mismo que "Gestión de
    // Ligas"), para que el header refleje la liga nueva y no la anterior.
