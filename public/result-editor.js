...
(function(global){
 ...
 function createModel(c,id){const ex=c.a&&c.b?existingFor(c):null;const stored=ex?copy(ex):null;const ns=ex?names(ex):[];let sets=ex?copy(ex.sets||[]):[];if(ns[0]===c.b&&ns[1]===c.a)sets=sets.map(([a,b])=>[b,a]);
  return{id,c:{...c,editId:ex?.id??c.editId},original:stored,league:_ligaActual,key:_saveSessionKey(),club:ex?.club||'',date:ex?.date||today(),sets:[sets[0]||['',''],sets[1]||['','']],stb:sets[2]?(sets[2][0]===1?'a':'b'):'',mode:ex?.wo?(ex.sets?.length?'ret':'wo'):'normal',loser:ex?.retiroDe||'',dirty:false,saving:false,host:null};}
 function readModel(m){if(!m.host)return;const q=n=>m.host.querySelector('[data-field="'+n+'"]');for(const k of ['club','date','loser','stb'])if(q(k))m[k]=q(k).value;for(let i=0;i<2;i++)for(let j=0;j<2;j++)if(q('s'+i+j))m.sets[i][j]=q('s'+i+j).value;}
 ...
 function validation(m){
  ...
  if(split){if(!m.stb)return{key:'valid_need_stb',field:'stb'};sets.push(m.stb==='a'?[1,0]:[0,1]);}
 ...
 function snapshotForReview(m){const v=validation(m);return{v,score:m.mode==='wo'?'W.O.':m.sets.map(s=>s.map(v=>v===''?'—':v).join('–')).join(' / ')+(m.stb&&m.mode==='normal'?' / '+(m.stb==='a'?'1–0':'0–1'):'')+(m.mode==='ret'?' · RET':'')};}
 function update(m){
  const root=m.host;if(!root?.isConnected)return;const ints=m.sets.map(s=>s.map(v=>v===''?NaN:Number(v)));const split=m.mode==='normal'&&ints.every(s=>SohailScore.validSet(...s))&&(ints[0][0]>ints[0][1])!==(ints[1][0]>ints[1][1]);
  const stb=root.querySelector('.re-stb');if(stb)stb.hidden=!split;if(!split){m.stb='';const sel=root.querySelector('[data-field="stb"]');if(sel)sel.value='';}
 ...
 function draw(m,host,isModal){
  ...
  const num=(i,j)=>'<label for="'+id+'-s'+i+j+'" class="ui-sr-only">'+e([c.a||t('re_a'),c.b||t('re_b')][j])+' · Set '+(i+1)+'</label><input id="'+id+'-s'+i+j+'" data-field="s'+i+j+'" type="number" min="0" max="7" step="1" inputmode="numeric" placeholder="0" value="'+e(m.sets[i][j])+'">';
  host.innerHTML='<div class="re-heading"><span class="ui-brand-mark">'+SohailUI.icon('matches')+'</span><div><h2 id="'+id+'-title">'+e(t(m.original?'re_edit':'re_title'))+'</h2><p>'+e(t('re_intro'))+'</p></div>'+(isModal?'<button type="button" class="re-close" data-close aria-label="'+e(t('close'))+'">'+SohailUI.icon('close')+'</button>':'')+'</div><p class="re-scope"></p>'+chooseHtml+
   '<div class="re-metadata"><div><label for="'+id+'-club">'+e(t('re_club'))+' <span aria-hidden="true">*</span></label><select id="'+id+'-club" data-field="club" required aria-describedby="'+id+'-club-help">'+opt('',t('re_choose_club'),m.club)+CLUBS.map(cl=>opt(cl.name,cl.name,m.club)).join('')+'</select><p id="'+id+'-club-help" class="re-help">'+e(t('re_club_help'))+'</p></div><div><label for="'+id+'-date">'+e(t('re_date'))+' <span aria-hidden="true">*</span></label><input id="'+id+'-date" type="date" data-field="date" required value="'+e(m.date)+'"></div></div>'+
   ...
   '<div class="re-score"><h3>'+e(t('re_score'))+'</h3><p class="re-help">'+e(t('re_format'))+'</p><div class="re-score-grid"><span></span><span class="re-set-label">Set 1</span><span class="re-set-label">Set 2</span><strong data-player="a">'+e(c.a||t('re_a'))+'</strong><div>'+num(0,0)+'</div><div>'+num(1,0)+'</div><strong data-player="b">'+e(c.b||t('re_b'))+'</strong><div>'+num(0,1)+'</div><div>'+num(1,1)+'</div></div><p class="re-help re-tb-info">'+e(t('re_tb_help'))+'</p></div>'+
   '<section class="re-stb" hidden><h3>'+SohailUI.icon('league')+e(t('re_stb'))+'</h3><p>'+e(t('re_stb_help'))+'</p><label for="'+id+'-stb">'+e(t('re_stb_winner'))+'</label><select id="'+id+'-stb" data-field="stb">'+opt('',t('re_choose_winner'),m.stb)+opt('a',(c.a||t('re_a'))+' · 1–0',m.stb)+opt('b',(c.b||t('re_b'))+' · 0–1',m.stb)+'</select></section>'+
 ...
 function showError(m,key,field){const error=m.host.querySelector('.re-error');error.hidden=false;error.textContent=t(key);if(field){const el=m.host.querySelector('[data-field="'+field+'"]');if(el){el.setAttribute('aria-invalid','true');el.focus();return;}}error.focus();}
 ...
 function open(input){
  ...
  dlg.showModal();host.querySelector('[data-field="club"]')?.focus({preventScroll:true});
 }
 ...
 async function save(m,modal){
  ...
  const saved=await SohailUI.mutation(()=>{
   ...
   if(c.po&&admin){storePo(c.ti,c.which,c.a,c.b,record.sets,record.winner,record.wo);rebuildTramo(c.ti);}
  ...
  else if(m.host.isConnected)showError(m,_saveConflict?'fix_conflict':'re_failed');
 }
