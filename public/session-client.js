...
(function(root,factory){
 ...
 function create({fetcher,storage=null,accept=()=>true,busy=()=>{},error=()=>{},timeout=15000}){
  let generation=0,controller=null,pending=null,logoutPending=Promise.resolve();
  ...
  async function request(action,signal){
   const res=await fetcher('/api/login',{method:'POST',headers:{'Content-Type':'application/json','X-Sohail-Session':'1'},
    credentials:'same-origin',cache:'no-store',body:JSON.stringify({accion:action}),signal,keepalive:action==='session-logout'});
   const data=await res.json();
   if(!res.ok){if(res.status===401)return {authenticated:false};throw new Error(data.code||'SESSION_UNAVAILABLE');}
   return data;
  }
  function logout(){
   cancel();put(DISABLED,'1');put(HINT,null);
   const ctl=new AbortController(),timer=setTimeout(()=>ctl.abort(),timeout);
   logoutPending=request('session-logout',ctl.signal).catch(()=>null).finally(()=>clearTimeout(timer));
   return logoutPending;
  }
  async function beforeLogin(){cancel();await logoutPending;}
  function restore(){
