...
(function(global){
 ...
 function phase(m){return m.po?(m.tLabel?t('draw')+' '+m.tLabel+' · ':'')+t(m.which==='cons'?'mh_cons':'mh_main'):t('cycle')+' '+m.cycle+' · '+groupName(m.g);}
 ...
 function statHTML(list,name){
  ...
  if(name){
   ...
   h+=section('mh_form','<div class="mh-form">'+(last.map(m=>'<button type="button" data-mh-open="'+e(m.id)+'" class="'+(D.winner(m)===name?'mh-form-win':'mh-form-loss')+'" aria-label="'+e(t(D.winner(m)===name?'mh_win':'mh_loss')+' · '+D.players(m).find(n=>n!==name)+' · '+formatDate(m.date))+'">'+e(t(D.winner(m)===name?'win_short':'loss_short'))+'</button>').join('')||'<span>'+e(t('mh_none'))+'</span>')+'</div><p class="mh-note">'+e(t('mh_newest'))+'</p>');
 ...
 global.SohailHistory=Object.freeze({render});
